...
const Products = () => {
  ...
  return (
    <section
      ref={containerRef}
      className='bg-gradient-to-b from-background via-background/50 to-transparent rounded-t-4xl shadow-elevation-lg border border-foreground/10'
    >
      {products.map((product, idx) => (
        <div
          ...
          className={`min-h-screen flex justify-center items-center px-8 lg:px-16 gap-12 py-20 relative ${idx === 0 ? '-mt-24' : ''
            }`}
        >
          {/* Animated gradient background */}
          <div
            className={`absolute inset-0 bg-gradient-to-r ${product.color} opacity-0 -z-10 rounded-2xl`}
          />
          ...
          >
            <div className='relative w-full max-w-lg'>
              {/* Glow effect */}
              <div className='absolute -inset-4 bg-gradient-to-br from-primary/30 to-accent/30 rounded-3xl blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-300' />

              {/* Image */}
              <img
                src={product.image}
                alt={`${product.name} - Premium Wireless Earbuds`}
                className='relative w-full aspect-square object-contain rounded-2xl shadow-elevation-lg transition-transform duration-300'
              />
            </div>
          </div>
